...
<schema xmlns="http://purl.oclc.org/dsdl/schematron"
	...
	<!--TODO should we check the CD has at least one index ?-->
	<!--<pattern id="index-ocurrence">
        <rule context="h:html">
            <assert test="count(.//*[tokenize(@epub:type,'/s+')='index']) = 1">At least one 'index'
	...
	<pattern id="index">
		<rule context="h:*[tokenize(@epub:type,'\s+')='index']">
			...
			<assert test="count(h:h1|h:h2|h:h3|h:h4|h:h5|h:h6) le 1">At most one heading element
				child must be present.</assert>
			<assert test="count(*[tokenize(@epub:type,'\s+')='index-headnotes']) le 1">At most one
				'index-headnotes' child must be present.</assert>
			<assert
				test="empty(h:* except (h:*[tokenize(@epub:type,'\s+')=('index-headnotes','index-entry-list','index-group','pagebreak')]
                                       |h:ul[not(@epub:type)]|h:h1|h:h2|h:h3|h:h4|h:h5|h:h6))"
				>No element other than a heading, 'index-headnotes', 'index-entry-list',
				'index-group' or 'pagebreak' is allowed as a child of 'index'.</assert>
			<assert
				test="if (h:*[tokenize(@epub:type,'\s+')='index-group'])
                      then empty(h:*[tokenize(@epub:type,'\s+')='index-entry-list']|h:ul)
                      else count(h:*[tokenize(@epub:type,'\s+')='index-entry-list']|h:ul)=1"
	...
	<pattern id="index-headnotes">
		<rule context="h:*[tokenize(@epub:type,'\s+')='index-headnotes']">
			<assert test="self::h:header|self::h:section|self::h:article|self::h:aside|self::h:nav"
				...
				elements.</assert>
			<assert test="parent::h:*[tokenize(@epub:type,'\s+')='index']">The 'index-headnotes'
	...
	<pattern id="index-group">
		<rule context="h:*[tokenize(@epub:type,'\s+')='index-group']">
			...
			<assert test="parent::h:*[tokenize(@epub:type,'\s+')='index']">The 'index-group' type is
				only allowed on children of 'index'.</assert>
			<assert test="count(h:h1|h:h2|h:h3|h:h4|h:h5|h:h6) le 1">'index-group' must not have
				more than one heading child.</assert>
			<assert
				test="count(h:*[tokenize(@epub:type,'\s+')='index-entry-list']|h:ul[not(@epub-type)])=1"
				>'index-group' must have exactly one 'index-entry-list' child.</assert>
			<assert
				test="empty(h:* except (h:*[tokenize(@epub:type,'\s+')=('pagebreak','index-entry-list')]
                                       |h:ul[not(@epub:type)]|h:h1|h:h2|h:h3|h:h4|h:h5|h:h6))"
				>No element other than a heading, an 'index-entry-list' or 'pagrebreak' is allowed
				as a child of 'index-group'.</assert>
		</rule>
	</pattern>
	...
	<pattern id="index-entry-list">
		<rule
			...
			<assert test="self::h:ul">The 'index-entry-list' type is only allowed on 'ul'
				elements.</assert>
			<assert
				test="parent::h:*[tokenize(@epub:type,'\s+')=('index','index-group','index-entry')]
                         |parent::h:li[parent::h:ul
	...
	<pattern id="index-entry">
		<rule
			...
			<assert
				test="count(h:*[tokenize(@epub:type,'\s+')='index-term' 
				                and not(tokenize(@epub:type,'\s+')=('index-xref-related','index-xref-preferred'))])=1"
				>An 'index-entry' must have one and only one 'index-term' child.</assert>
			<assert
				test="exists(h:ul[not(@epub:type)]
                            |h:*[tokenize(@epub:type,'\s+')=('index-entry-list','index-editor-note','index-locator-list','index-locator',
                                                             'index-xref-preferred','index-xref-related')])"
				>An 'index-entry' must have at least one child with the type 'index-entry-list' or
				'index-locator-list' or 'index-locator' or 'index-editor-note' or
			...
			<assert
				test="if (h:*[tokenize(@epub:type,'\s+')='index-locator-list']) 
                      then empty(h:*[tokenize(@epub:type,'\s+')='index-locator'])
                      else true()"
			...
			<assert test="count(h:*[tokenize(@epub:type,'\s+')='index-locator-list']) le 1">An
				'index-entry' must have at most one child with the type
				'index-locator-list'.</assert>
			<assert test="count(h:*[tokenize(@epub:type,'\s+')='index-editor-note']) le 1">An
				'index-entry' must have at most one child with the type
				'index-editor-note'.</assert>
			<assert
				test="if (h:*[tokenize(@epub:type,'\s+')='index-xref-preferred']) 
                      then empty(h:*[tokenize(@epub:type,'\s+')='index-xref-related']) 
                      else true()"
				>An 'index-entry' must not have both 'index-xref-preferred' and 'index-xref-related'
				children.</assert>
			<assert
				test="empty(h:* except (h:*[tokenize(@epub:type,'\s+')=('index-term','index-entry-list','index-editor-note',
	...
	<pattern abstract="true" id="index-entry-child">
		<rule context="h:*[tokenize(@epub:type,'\s+')='$type']">
			<assert
				test="parent::h:*[tokenize(@epub:type,'\s+')='index-entry']
                |parent::h:li[parent::h:ul
                [tokenize(@epub:type,'\s+')='index-entry-list' 
                or (not(@epub:type)
                and ancestor::h:*[tokenize(@epub:type,'\s+')='index'] 
		...
		<rule context="h:*[tokenize(@epub:type,'\s+')='index-term']">
			...
			<assert
				test="parent::h:*[tokenize(@epub:type,'\s+')=('index-entry','index-xref-related','index-xref-preferred')]
				     |parent::h:li[parent::h:ul[tokenize(@epub:type,'\s+')='index-entry-list' 
				                                or (not(@epub:type)
				                                    and ancestor::h:*[tokenize(@epub:type,'\s+')='index'] 
				                                    and empty(ancestor::h:*[tokenize(@epub:type,'\s+')='index-headnotes']))]]"
	...
	<pattern id="index-locator-list">
		<rule context="h:*[tokenize(@epub:type,'\s+')='index-locator-list']">
			<assert test="self::h:ul">The 'index-locator-list' type is only allowed on 'ul'
				elements.</assert>
			<assert test="parent::h:*">The 'index-locator-list' type is only allowed on children of
				'index-entry'.</assert>
			<assert
		...
		<rule
			...
			<assert
				test="ancestor::h:*[tokenize(@epub:type,'\s+')=('index-locator-list','index-locator-range')]
                      |parent::h:*[tokenize(@epub:type,'\s+')='index-entry']
                      |parent::h:li[parent::h:ul[tokenize(@epub:type,'\s+')='index-entry-list' 
                                                 or (not(@epub:type)
                                                     and ancestor::h:*[tokenize(@epub:type,'\s+')='index'] 
                                                     and empty(ancestor::h:*[tokenize(@epub:type,'\s+')='index-headnotes']))]]"
	...
	<pattern id="index-locator-range">
		<rule context="h:*[tokenize(@epub:type,'\s+')='index-locator-range']">
			<!--FIXME check the validity of this rule-->
			<assert
